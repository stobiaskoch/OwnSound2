...
if($_REQUEST['order']=="search") {
	...
	if ($result = $mysqli->query($query)) {
		while ($row = $result->fetch_assoc()) {
			...
			$rows[] =  "<a href='#' onclick='albumlist(".$row['id'].")'><font size='2px' color='#4faac6'><b>" . utf8_encode($row['name']) . "</font></b></a> - $albumcount";
		}
		$result->free();
	}
	...
	if ($result = $mysqli->query($query)) {
		while ($row = $result->fetch_assoc()) {
			...
			$albums[] =  "<a href='#' onclick='tracklist(".$row['id'].")'><font size='2px' color='#4faac6'><b>" . utf8_encode($artist) . "</font></b> - " . utf8_encode($row['name']) . "</a>";
		}
		$result->free();
	}
